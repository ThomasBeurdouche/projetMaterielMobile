...
class PaysListActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            recyclerView.adapter = PaysAdapter(withContext(Dispatchers.IO){apiManager.getPays()}, "PaysListActivity" )
            animation.dismiss()
        }
        ...
        barreRecherche.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            override fun onQueryTextSubmit(query: String?): Boolean {
                ...
                CoroutineScope(Dispatchers.Main).launch {
                    recyclerView.layoutManager = LinearLayoutManager(this@PaysListActivity, LinearLayoutManager.VERTICAL, false)
                    recyclerView.adapter = PaysAdapter(withContext(Dispatchers.IO){apiManager.getPaysByName(query)}, "PaysListActivity")
                    animation.dismiss()
                }
